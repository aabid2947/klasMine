...
export default function ImagesToImages({
  ...
  // Populate form with customization data when available
  useEffect(() => {
    if (customizationData) {
      setFormData(prev => ({
        ...prev,
        productName: customizationData.name || prev.productName,
        description: customizationData.description || prev.description,
        price: customizationData.price || prev.price,
      }));
    }
  }, [customizationData]);
  ...
  const fetchFilters = async () => {
    try {
      ...
      const res = await postRequest("/post/form", payload, true); // Add withToken: true
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Use /post/save for saving (as discussed earlier)
      const res = await postRequest("/post/save", payload, true); // Add withToken: true
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      {/* Main Images */}
      <div className="flex flex-col md:flex-row mb-4 gap-3 w-full">
        {/* Original Image */}
        <div className="w-full md:w-1/2">
          {originalImage ? (
            <div className="relative">
              <div
                className="aspect-square w-full max-w-xs mx-auto overflow-hidden rounded-md bg-gray-100"
            ...
            <div
              className="aspect-square w-full max-w-xs mx-auto bg-gray-200 rounded-md flex items-center justify-center"
              style={{ maxWidth: "288px", maxHeight: "288px" }}
            >
        ...
        <div className="w-full md:w-1/2 relative">
          <div className="relative">
            ...
            <p className="text-xs text-gray-500 mt-1 text-center">
              {Array.isArray(generatedImages) && generatedImages.length > 0
                ? `Generated Image ${Array.isArray(generatedImages) && generatedImages.length > 1
              ...
              <div className="">
                <div className="px-4 sm:px-6 py-4 bg-[rgba(241,242,244,1)] sticky top-0 z-10">
                  ...
                  <button
                    ...
                    className="absolute top-5 right-5 sm:right-7 text-black text-2xl hover:text-gray-600 z-50"
                    aria-label="Close"
                  >
                    ✕
                  </button>
                ...
                <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 p-4 sm:p-6 lg:p-8">
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.currentTarget.src =
                                "data:image/svg+xml;base64,";
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <div className="w-full lg:w-[55%]">
                    ...
                    {/* Orientation: Multi-checkboxes (styled) */}
                    <div className="mb-3 sm:mb-4">
                        ...
                        {orientations.map((o: any) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-colors text-sm ${checked
      ...
      <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 justify-items-center">
          ...
          {/* Locked Thumbnails (placeholder for future generations) */}
          {(!Array.isArray(generatedImages) || generatedImages.length < 4) &&
            Array.from({
              length: 4 - (Array.isArray(generatedImages) ? generatedImages.length : 0),
            }).map((_, index) => (
              <div
                key={`locked-${index}`}
      ...
      <div className="flex gap-4 justify-center items-center p-3 mt-4">
        {/* Only show Customize button - no Add to Cart in main section */}
        <button
          onClick={() => {
            if (
              !Array.isArray(generatedImages) ||
    ...
    */}
                  ...
                  <div className="space-y-4">
                    ...
                    {showCustomizedImages ? (
                      ...
                      <button
                        ...
                        className={`px-3 py-3 font-semibold rounded-md transition ${!selectedArticle
                          ? "text-gray-400 bg-gray-300 cursor-not-allowed"
                          : "text-white bg-gradient-to-r from-purple-400 to-blue-500 hover:opacity-90"
                          }`}
                      >
      ...
      {isPreviewModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4 py-6">
          <div className="bg-white rounded-[20px] max-w-4xl w-full relative overflow-hidden shadow-2xl">
            <div className="p-6">
              {/* Header */}
