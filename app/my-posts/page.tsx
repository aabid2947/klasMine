...
const MyPosts = () => {
  ...
  // 🧩 Fetch my posts - fetch user's own posts with filters
  const fetchMyPosts = async () => {
    ...
    try {
      const res = await postRequest("/post/list", {
        page: "1",
        query: searchQuery,
        orientation_ids: appliedFilters.orientation_ids,
        sub_category_id: appliedFilters.sub_category_id,
        article_style_id: appliedFilters.article_style_id,
        article_type: appliedFilters.article_type,
      ...
      if (res?.success) {
        if (res?.data?.posts && Array.isArray(res.data.posts)) {
          ...
          const userPosts = res.data.posts.filter((post: MyPost) => post.user_id === user.user_id);
          console.log("My posts fetched:", userPosts);
        ...
        toast.error(res?.message || "Failed to fetch your posts");
        setAllPosts([]);
        setFilteredPosts([]);
      }
    } catch (err) {
  ...
  const applyFilters = () => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(post => 
        ...
        post.article_category_name.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
    ...
    if (appliedFilters.orientation_ids.length > 0) {
      filtered = filtered.filter(post => {
        ...
        if (!post.orientation_ids || !Array.isArray(post.orientation_ids) || post.orientation_ids.length === 0) {
          return false;
        }
        
        // Check if orientation_ids contains objects or strings
        return post.orientation_ids.some(orientationItem => {
    ...
    if (appliedFilters.article_style_id) {
      filtered = filtered.filter(post => 
        post.article_category_id === appliedFilters.article_style_id
      );
    }
    ...
    if (appliedFilters.sub_category_id) {
      filtered = filtered.filter(post => 
        post.sub_category_id === appliedFilters.sub_category_id
      );
    }

    // Apply article type filter
    if (appliedFilters.article_type) {
      // This would need to be added to Post interface if available in API response
      // For now, we'll skip this filter as it's not in the current post structure
    }
    ...
    console.log('Filtered posts:', filtered.length, 'out of', allPosts.length);
  ...
  useEffect(() => {
    applyFilters();
  }, [allPosts, searchQuery, appliedFilters]);
  ...
  // 🧩 Handle Apply Filters - refetch posts with new filters
  ...
  const handleSearch = (query: string) => {
    ...
    // Refetch posts with search query
    fetchMyPosts();
  };
            ...
            {(appliedFilters.orientation_ids.length > 0 || 
              ...
              <button 
                onClick={() => {
                  ...
                  setSearchQuery("");
                  fetchMyPosts(); // Refetch without filters
                }}
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">
              {searchQuery || appliedFilters.orientation_ids.length > 0 || appliedFilters.article_style_id || appliedFilters.sub_category_id || appliedFilters.article_type || appliedFilters.price
                ? "Try adjusting your search or filters to find your posts."
                : "You haven't created any posts yet. Start creating to see them here!"}
            </p>
            {(searchQuery || appliedFilters.orientation_ids.length > 0 || appliedFilters.article_style_id || appliedFilters.sub_category_id || appliedFilters.article_type || appliedFilters.price) && (
              <button
                onClick={() => {
